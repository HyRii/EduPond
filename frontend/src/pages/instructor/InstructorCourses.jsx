...
const InstructorCourses = () => {
  ...
  const handleSubmit = async (courseId) => {
    const confirmed = window.confirm("Submit this course for review?");
    if (!confirmed) return;
  ...
  const handleDelete = async (courseId) => {
    const confirmed = window.confirm("Delete this draft course?");
    if (!confirmed) return;
  ...
  // Fase 3A:
  // DRAFT, REJECTED, PUBLISHED -> boleh edit
  // PENDING_REVIEW, ARCHIVED -> tidak boleh edit
    ...
    <section className="instructor-page">
      <div className="instructor-page-header">
        <div>
          ...
          <h1>My Courses</h1>
          <p className="page-description">Create and manage your courses.</p>
        </div>

        <Link to="/instructor/courses/new" className="primary-link">
          Create Course
        </Link>
      </div>
      ...
      {loading ? (
        <div className="instructor-loading">Loading your courses...</div>
      ) : courses.length === 0 ? (
        <EmptyState
        ...
        <div className="instructor-list">
          {courses.map((course) => (
            <article key={course.id} className="instructor-list-item">
              ...
              <div className="instructor-actions">
                ...
                {canSubmit(course.status) && (
                  <button type="button" onClick={() => handleSubmit(course.id)}>
                    Submit Review
                  </button>
                )}
                ...
                {course.status === "PENDING_REVIEW" && (
                  <span className="course-review-note">
                ...
                {course.status === "PUBLISHED" && (
                  <span className="course-review-note">
                    Editing will send this course back for review
                  </span>
                )}
              </div>
            </article>
